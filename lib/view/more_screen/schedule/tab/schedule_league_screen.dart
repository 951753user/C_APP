...
class ScheduleLeagueScreen extends StatefulWidget {
  const ScheduleLeagueScreen({Key? key}) : super(key: key);

  @override
  State<ScheduleLeagueScreen> createState() => _ScheduleLeagueScreenState();
}

class _ScheduleLeagueScreenState extends State<ScheduleLeagueScreen> {
  ...
  void dispose() {
    bannerLoad = false;
    controller.noMoreLeagueData.value = false;
    super.dispose();
  }
  ...
  void _getData() async {
    ...
    if (scrollindex > 2) {
      if (controller.scrollLeagueFirstId.value !=
          controller.scrollLeagueLastId.value) {
        controller
            .getScheduleLeagueMoreData(controller.scrollLeagueLastId.value);
      }
      // debugPrint(
      //     "this is scroll take ---------------${newsController.scrollNewsLastId.value} -------------$scrollindex");
    } else {
      controller.getScheduleLeagueMoreData(controller.leagueLastId.value);
      // print("123456789==================================================");
      // debugPrint(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Flexible(
                  child: FutureBuilder(
                    future: controller.getScheduleLeagueData(),
                                          ...
                                          Flexible(
                                            child: ListView.builder(
                                              ...
                                              itemBuilder: (context, md) {
                                                // controller.leagueLastId.value = "${data.matchScheduleMap![i].scheduleAdWrapper?.matchScheduleList?.last.matchInfo?.last.endDate}";
                                                controller.leagueLastId.value = "${data.matchScheduleMap?.last.scheduleAdWrapper?.matchScheduleList?.last.matchInfo?.last.endDate}";
                ...
                Obx(
                    ...
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: controller.moreScheduleLeagueData.length + 1,
                    itemBuilder: (context, i) {
                      if(i < controller.moreScheduleLeagueData.length){
                        return Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Container(
                              ...
                              child: Padding(
                                padding: const EdgeInsets.all(15.0),
                                child: Text(
                                  "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.date}",
                                  overflow: TextOverflow.ellipsis,
                                  style: GoogleFonts.poppins(
                            ...
                            Flexible(
                              child: ListView.builder(
                                shrinkWrap: true,
                                physics: NeverScrollableScrollPhysics(),
                                itemCount: controller.moreScheduleLeagueData[i]
                                      ...
                                      InkWell(
                                        onTap: () {
                                          Get.to(
                                                () => MatchSeriesDetailsScreen(
                                                seriesName:
                                                "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList![index].seriesName}",
                                                seriesId:
                                                "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList![index].seriesId}"),
                                          );
                                        },
                                        child: Container(
                                          width: double.infinity,
                                      ...
                                      Flexible(
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, md) {
                                            ...
                                            controller.scrollLeagueFirstId.value = "${controller.moreScheduleLeagueData.first.scheduleAdWrapper?.matchScheduleList.first.matchInfo?.first.endDate}";
                                            controller.scrollLeagueLastId.value = "${controller.moreScheduleLeagueData.last.scheduleAdWrapper?.matchScheduleList.last.matchInfo?.last.endDate}";

                                            var dt = DateTime.fromMillisecondsSinceEpoch(
                                                int.parse(
                                                    "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].startDate}"));
...
// 12 Hour format:
                                            ...
                                            return InkWell(
                                              onTap: () {
                                                ...
                                                Get.to(MatchDetailsScreen(
                                                  Id: '${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].matchId}',
                                                  upcoming: true,
                                                  t1: '${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team1?.teamSName}',
                                                  t2: '${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team2?.teamSName}',
                                                ));
                                              },
                                              child: Card(
                                                elevation: 2,
                                                  ...
                                                  child: Column(
                                                    ...
                                                    children: [
                                                            ...
                                                            InkWell(
                                                              onTap: () async {
                                                                ...
                                                                compareReminderList.add({
                                                                  "$ptime":
                                                                  "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team1?.teamSName} vs ${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team2?.teamSName} at ${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].venueInfo?.ground}"
                                                                });
                                                        ...
                                                        children: [
                                                          Padding(
                                                            ...
                                                            child: Row(
                                                              children: [
                                                                Image.network(
                                                                  controller.getImage(
                                                                      "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team1?.imageId}"),
                                                                  height: 40.h,
                                                                  width: 40.w,
                                                                ),
                                                                SizedBox(
                                                                ...
                                                                Text(
                                                                    "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team1?.teamSName}",
                                                                    style: GoogleFonts
                                                                        .poppins(
                                                                      fontSize: 15.sp,
                                                                      fontWeight:
                                                          ...
                                                          Padding(
                                                            ...
                                                            child: Row(
                                                              children: [
                                                                Text(
                                                                    "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team2?.teamSName}",
                                                                    style: GoogleFonts
                                                                        .poppins(
                                                                      fontSize: 16.sp,
                                                                      fontWeight:
                                                                ...
                                                                Image.network(
                                                                  controller.getImage(
                                                                      "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].team2?.imageId}"),
                                                                  height: 40.h,
                                                                  width: 40.w,
                                                                ),
                                                              ],
                                                      ...
                                                      Padding(
                                                        ...
                                                        child: Text(
                                                          "${controller.moreScheduleLeagueData[i].scheduleAdWrapper?.matchScheduleList[index].matchInfo[md].venueInfo?.ground}",
                                                          style: GoogleFonts.poppins(
                                                              fontSize: 12,
                                                              fontWeight:
                                                              FontWeight.w600),
                        ...
                        return Obx(() => controller.noMoreLeagueData.value == true?Container():Padding(
